...
export default function PlatformAdminPage() {
  ...
  const cards = [
    { icon: Building2,   label: 'Companies',       value: stats.total,                              sub: `${stats.active} active`,        tint: '#16A34A' },
    ...
    { icon: CreditCard,  label: 'Paid Customers',  value: stats.paid,                               sub: `of ${stats.total} companies`,   tint: '#B45309' },
  ]
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#f1f5f9' }}>
      ...
      <header style={{
        background: 'linear-gradient(90deg, #0B3D1E, #0A2E17)', color: '#fff', padding: '0 24px', height: 60,
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <div>
            <div className="tg-display" style={{ fontSize: 15, fontWeight: 700 }}>Platform Console</div>
            <div style={{ fontSize: 12, color: 'rgba(255,255,255,0.5)' }}>Taurus Go · Owner</div>
          </div>
        </div>
      ...
      <main style={{ maxWidth: 1100, margin: '0 auto', padding: '28px 24px' }}>
        ...
        <p style={{ color: '#64748b', fontSize: 15, marginTop: 6 }}>
          Manage every company using Taurus Go from here.
        </p>
        ...
        <div style={{
          ...
          <div style={{ fontSize: 15, fontWeight: 700, color: '#0f172a' }}>Plan breakdown</div>
          <div style={{ fontSize: 13, color: '#94a3b8', marginTop: 2, marginBottom: 16 }}>
          ...
          <div style={{ display: 'flex', gap: 22, marginTop: 14, flexWrap: 'wrap' }}>
            {Object.keys(PLANS).map(key => (
              <div key={key} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                ...
                <span style={{ fontSize: 14, color: '#334155', fontWeight: 500 }}>{PLANS[key].label}</span>
                <span style={{ fontSize: 14, color: '#94a3b8' }}>{stats.byPlan[key] || 0}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div style={{ fontSize: 15, fontWeight: 700, color: '#0f172a', marginRight: 'auto' }}>All companies</div>
            ...
            <div style={{ display: 'flex', gap: 4, background: '#f1f5f9', borderRadius: 9, padding: 3 }}>
              {['all', 'active', 'suspended'].map(s => (
                <button key={s} onClick={() => setStatusFilter(s)} style={{
                  fontSize: 13.5, fontWeight: 600, textTransform: 'capitalize', border: 'none', cursor: 'pointer',
          ...
          <div style={{ overflowX: 'auto' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 720 }}>
                ...
                <tr style={{ background: '#f8fafc', borderBottom: '1px solid #f1f5f9' }}>
                  {['Company', 'Plan', 'Users', 'Status', 'Joined', ''].map((h, i) => (
                    <th key={i} style={{
                      ...
                      fontSize: 12, fontWeight: 600, color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '0.4px',
                    }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {filtered.length === 0 ? (
                  <tr><td colSpan={6} style={{ padding: '40px', textAlign: 'center', color: '#94a3b8', fontSize: 14 }}>
                  ...
                  return (
                    ...
                    >
                      <td style={{ padding: '12px 22px' }}>
                        <div style={{ fontSize: 14.5, fontWeight: 600, color: '#0f172a' }}>{c.name}</div>
                        <div style={{ fontSize: 13, color: '#94a3b8' }}>{c.domain}</div>
                      </td>
                      <td style={{ padding: '12px 22px' }}>
                        <span style={{
                          fontSize: 13, fontWeight: 600, color: plan.color, background: plan.bg,
                          padding: '3px 10px', borderRadius: 20,
                        }}>{plan.label}</span>
                      </td>
                      <td style={{ padding: '12px 22px', textAlign: 'right', fontSize: 14, color: '#334155' }}>{c.seats}</td>
                      <td style={{ padding: '12px 22px' }}>
                        <span style={{
                      ...
                      <td style={{ padding: '12px 22px', fontSize: 14, color: '#64748b' }}>
          ...
          <div onClick={e => e.stopPropagation()} style={{
            ...
            <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between' }}>
              <div>
                ...
                <div style={{ fontSize: 14, color: '#94a3b8' }}>{selected.domain}</div>
              </div>
            ...
            <div style={{ marginTop: 24 }}>
              <label style={{ fontSize: 13, fontWeight: 600, color: '#64748b', display: 'block', marginBottom: 6 }}>PLAN</label>
              <select value={selected.plan} onChange={e => changePlan(selected.id, e.target.value)} style={{
                ...
                fontSize: 14.5, color: '#0f172a', background: '#fff', cursor: 'pointer',
              }}>
                {Object.keys(PLANS).map(key => (
...
function DetailRow({ label, value }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
      ...
      <span style={{ fontSize: 14.5, fontWeight: 600, color: '#0f172a' }}>{value}</span>
    </div>
  )
}
