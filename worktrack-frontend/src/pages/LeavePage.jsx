...
const typeStyle   = {
  ...
  CASUAL:    { background: '#eff6ff', color: '#2563eb' },
  ...
  return (
    <div style={{ width: '100%' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 20 }}>
        <div>
          <h1 style={{ fontSize: 20, fontWeight: 600, color: '#0f172a', letterSpacing: '-0.3px' }}>Leave Requests</h1>
          <p style={{ fontSize: 13, color: '#64748b', marginTop: 4 }}>Manage employee leave applications</p>
        </div>
      ...
      {showForm && (
        <div style={{ background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: 12, padding: '20px', marginBottom: 16 }}>
          <h3 style={{ fontSize: 14, fontWeight: 600, color: '#0f172a', marginBottom: 16 }}>New Leave Application</h3>
          ...
          {applyMutation.isError && (
            <p style={{ fontSize: 12, color: '#ef4444', marginTop: 10 }}>Failed to submit. Check all fields.</p>
          )}
          ...
          <div style={{ display: 'flex', gap: 10, marginTop: 16 }}>
            <button
              ...
              style={{ padding: '9px 20px', borderRadius: 8, border: 'none', fontSize: 13, fontWeight: 500, background: '#4f46e5', color: '#fff', cursor: 'pointer' }}
            >
      ...
      <div style={{ background: '#fff', border: '1px solid #e2e8f0', borderRadius: 12, overflow: 'hidden' }}>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 120px 140px 100px 1fr', padding: '12px 20px', borderBottom: '1px solid #f1f5f9', background: '#f8fafc' }}>
        ...
        {isLoading ? (
          ...
          <div style={{ padding: '60px 24px', textAlign: 'center' }}>
            ...
            <p style={{ fontSize: 14, fontWeight: 500, color: '#334155', margin: 0 }}>No leave requests</p>
          ...
          leaves.map((l, i) => {
            ...
            return (
              <div key={l.id}>
                <div style={{
                  display: 'grid', gridTemplateColumns: '1fr 120px 140px 100px 1fr',
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                    ...
                    <div>
                      ...
                      <p style={{ fontSize: 11, color: '#94a3b8', margin: '2px 0 0' }}>{l.totalDays} day{l.totalDays !== 1 ? 's' : ''}</p>
                    </div>
                  </div>

                  {/* Leave type */}
                  <span style={{ fontSize: 11, fontWeight: 600, padding: '3px 10px', borderRadius: 6, ...ts }}>
                    {typeLabel[l.leaveType] || l.leaveType}
                  </span>

                  {/* Dates */}
                  <div>
                    <p style={{ fontSize: 12, color: '#334155', margin: 0 }}>{l.startDate}</p>
                    {l.endDate !== l.startDate && (
                      <p style={{ fontSize: 11, color: '#94a3b8', margin: '2px 0 0' }}>→ {l.endDate}</p>
                    )}
                  </div>

                  {/* Status */}
                  <span style={{ fontSize: 11, fontWeight: 600, padding: '3px 10px', borderRadius: 6, ...ss }}>
                    {l.status}
                  </span>
                  ...
                  <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
                    {l.status === 'PENDING' && (
                      <>
                        <button
                          ...
                          style={{ fontSize: 12, padding: '5px 12px', borderRadius: 6, border: '1px solid #bbf7d0', background: '#f0fdf4', color: '#16a34a', cursor: 'pointer', whiteSpace: 'nowrap' }}
                        >
                          Approve
                        </button>
                        <button
                          ...
                          style={{ fontSize: 12, padding: '5px 12px', borderRadius: 6, border: '1px solid #fecaca', background: '#fff', color: '#dc2626', cursor: 'pointer', whiteSpace: 'nowrap' }}
                        >
                          Reject
                        </button>
                      </>
                    ...
                    {l.managerComment && (
                      <span style={{ fontSize: 11, color: '#64748b' }} title={l.managerComment}>
                        "{l.managerComment.length > 20 ? l.managerComment.slice(0, 20) + '...' : l.managerComment}"
                      </span>
                    )}
                  </div>
                </div>
                ...
                {isActing && (
                  <div style={{ background: '#f0fdf4', borderBottom: '1px solid #bbf7d0', padding: '12px 20px', display: 'flex', gap: 10, alignItems: 'center' }}>
                    <input
                      ...
                      style={{ flex: 1, padding: '7px 12px', borderRadius: 8, border: '1px solid #bbf7d0', fontSize: 13 }}
                    />
                    <button
                      onClick={() => approveMutation.mutate({ id: l.id, comment })}
                      disabled={approveMutation.isPending}
                      style={{ padding: '7px 16px', borderRadius: 8, border: 'none', background: '#16a34a', color: '#fff', fontSize: 13, fontWeight: 500, cursor: 'pointer' }}
                    >
...
const lbSt = { display: 'block', fontSize: 11, fontWeight: 600, color: '#64748b', marginBottom: 6 }
